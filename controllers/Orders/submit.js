...
const GetSocketRequestSession = require("../libs/GetSocketRequestSession");
...
module.exports = async function(io, socket, order){ 
    try{
        ...
        emitSuccessToLaundry(io, laundryInitials, orderID);

    }catch(err){
        console.log(err);
        emitFailureToLaundry(io, laundryInitials, {error: "Error"});
    } 
...
function emitSuccessToLaundry(io, laundryInitials, orderID){
    io.of("/laundry").to(laundryInitials).emit("submit-order-success", orderID);
    io.of("/laundry").to(laundryInitials).emit("update-orders");
}

function emitFailureToLaundry(io, laundryInitials, error){
    io.of("/laundry").to(laundryInitials).emit("submit-order-failure", error);
}
